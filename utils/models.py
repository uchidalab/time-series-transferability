...
def cnn_lenet(input_shape, nb_class):
    ...
    for i in range(nb_cnn):
        conv = Conv1D(6+10*i, 3, padding='same', activation="relu",
...
def cnn_vgg(input_shape, nb_class):
    ...
    for i in range(nb_cnn):
        ...
        conv = Conv1D(num_filters, 3, padding='same',
                      activation="relu", kernel_initializer='he_uniform')(conv)
        conv = Conv1D(num_filters, 3, padding='same',
                      activation="relu", kernel_initializer='he_uniform')(conv)
        if i > 1:
            conv = Conv1D(num_filters, 3, padding='same',
...
def lstm_fcn(input_shape, nb_class):
    ...
    conv = Conv1D(256, 5, padding='same',
                  kernel_initializer='he_uniform')(conv)
    conv = BatchNormalization()(conv)
    conv = Activation('relu')(conv)
...
def cnn_resnet(input_shape, nb_class):
    ...
    for i, nb_nodes in enumerate([64, 128, 128]):
        conv = Conv1D(nb_nodes, 8, padding='same',
                      kernel_initializer="glorot_uniform")(conv)
        conv = BatchNormalization()(conv)
        conv = Activation('relu')(conv)

        conv = Conv1D(nb_nodes, 5, padding='same',
                      kernel_initializer="glorot_uniform")(conv)
        conv = BatchNormalization()(conv)
        conv = Activation('relu')(conv)

        conv = Conv1D(nb_nodes, 3, padding='same',
                      kernel_initializer="glorot_uniform")(conv)
        conv = BatchNormalization()(conv)
        conv = Activation('relu')(conv)
        ...
        if i < 2:
            ...
            residual = Conv1D(nb_nodes, 1, padding='same',
                              kernel_initializer="glorot_uniform")(residual)
